...
describe('Module', function () {
    ...
    it('should call Resource Manager\'s loadTemplate with the template option passed in instantiation when load() is called', function () {
        ...
        return module.load().then(function () {
            assert.deepEqual(resourceManagerLoadTemplateStub.args[0], [templateUrl]);
            module.destroy();
        });
    });
    ...
    it('should insert the string contents of the template at the url provided in the options.template property into the module element on load() call', function () {
        var el = document.createElement("div");
        var template = 'template.html';
    ...
    it('should invoke handlebar template function with options data and inject the html resolved from the ResourceManager\'s loadTemplate method when load is called', function () {
        var templateData = {my: 'templateData'};
        var el = document.createElement("div");
        var templateUrl = 'template.hbs';
        var html = '<div class="my-template-html"></div>';
        var templateFunc = sinon.stub().returns(html);
        var handlebarsCompileStub = sinon.stub(Handlebars, 'compile');
        handlebarsCompileStub.returns(templateFunc);
        resourceManagerLoadTemplateStub.withArgs(templateUrl).returns(Promise.resolve(html));
        var module = new Module(el, {template: templateUrl, data: templateData});
        return module.load().then(function () {
            assert.deepEqual(templateFunc.args[0][0], templateData);
            assert.deepEqual(el.innerHTML, html);
            handlebarsCompileStub.restore();
            module.destroy();
        });
    });

    it('should invoke inject handlebar file contents to module el the same way they appear in the original file', function () {
        var el = document.createElement("div");
        var templateUrl = 'template.hbs';
        var html = '<div class="my-template-html"></div><div class="my-second-template-html"></div>';
        var templateFunc = sinon.stub().returns(html);
        var handlebarsCompileStub = sinon.stub(Handlebars, 'compile');
        handlebarsCompileStub.returns(templateFunc);
        resourceManagerLoadTemplateStub.withArgs(templateUrl).returns(Promise.resolve(html));
