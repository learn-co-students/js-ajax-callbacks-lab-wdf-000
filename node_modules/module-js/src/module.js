...
import slugify from 'handlebars-helper-slugify';
...
class Module {
    ...
    load () {
        ...
        if (!this.loaded) {
            ...
            // load all subModules
            return Promise.all(_.invoke(views, 'load'))
                .then(() => {
    ...
    getTemplate (data) {
        ...
        if (this._isHTMLTemplate(template)) {
            ...
            return ResourceManager.loadTemplate(template).then((html) => {
                html = html || '';
                if (html && isHandlebarFile(template)) {
                    // handle bar file
                    html = Handlebars.compile(html)(data || {});
                }
                return this.getTemplateContent(html);
            });
        }
    }
